...
#' @export
#' @source \url{https://www.mathsisfun.com/numbers/fibonacci-sequence.html}
#' @param x The number of numbers in the sequence.
#' @param round Decimals to print of the golden ratio, default is 9.
...
fibonacci <- function(x,round=9){
  a <- c(0,1)
  gold <- c(0,1)
  x <- x-2
  for (i in 1:x) {
    a[i+2] <- sum(a[i],a[i+1])
    gold[i+2] <- round(a[i+1]/a[i],round)
  }
