...
mitun <- function (DV, x.pred, x.mod, sdH = 1, sdM = 0, sdL = -1) 
{
    ...
    colL = paste0(sdL, " SD")
    colM <- ifelse(sdM == 0, "Mean", ifelse(sdM > 0, paste0("+", 
        sdM, " SD"), paste0(sdM, " SD")))
    library(ggplot2)
    ...
    gg <- g + stat_function(fun = funH, aes(color = SD_Moderator), 
        size = 0.8) + stat_function(fun = funM, aes(color = colM), 
        size = 0.6) + stat_function(fun = funL, aes(color = colL), 
        size = 0.4) + xlab("Predictor values")
    print(summary(m))
    print(data.frame(SD = c(sdH, sdM, sdL), intercepts = c(interH, 
        interM, interL), slopes = c(mH, mM, mL)))
